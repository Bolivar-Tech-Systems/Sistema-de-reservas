...
def create_permiso_endpoint(permiso: PermisoCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def update_permiso_endpoint(permiso_id: int, permiso: PermisoCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def get_permiso_endpoint(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return list_all_permisos(db)

@router.delete("/delete/{permiso_id}")
def delete_permiso_endpoint(permiso_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return delete_permiso(db, permiso_id)

@router.get("/user/{user_id}", response_model=list[PermisoResponse])
def list_permisos_by_user_endpoint(user_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return list_all_permisos_by_user(db, user_id)

@router.post("/role/{role_id}", response_model=PermisoRoleResponse)
def assing_permiso_to_role_endpoint(role_id: int, Permiso_role: PermisoRoleAssing, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def list_permisos_by_role_get_endpoint(role_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
